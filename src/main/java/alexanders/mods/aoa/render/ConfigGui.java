...
public class ConfigGui extends Gui {
    ...
    private static final AuthorizationCodeUriRequest authorizationCodeUriRequest = spotifyApi.authorizationCodeUri().state(Util.getSecureRandomState())
            .scope("user-modify-playback-state,user-read-playback-state").show_dialog(true).build();
    ...
    private boolean onLogin() {
        try {
            ...
            String state = null;
            if (properties.length != 2) throw new RuntimeException("Invalid response from spotify");
            String[] property = properties[0].split("=");
            if (property.length != 2) throw new RuntimeException("Invalid response from spotify");
            if (property[0].equals("code")) authCode = property[1];
            else if (property[0].equals("state")) state = property[1];
            property = properties[1].split("=");
            if (property.length != 2) throw new RuntimeException("Invalid response from spotify");
            if (property[0].equals("code")) authCode = property[1];
            else if (property[0].equals("state")) state = property[1];
            if (state == null || authCode == null) throw new RuntimeException("Invalid response from spotify");
            ...
            if (!file.exists()) if (!file.createNewFile()) throw new IOException("Couldn't create token storage file");
