...
public class Recipes {
    public static void init() {
        ...
        for (int i = 1; i < values.length; i++) {
            ItemInstance in = new ItemInstance(Tiles.plain, 1, i - 1);
            ItemInstance out = new ItemInstance(Tiles.plain, 1, i);
            new BasicRecipe(createRes("plain_" + values[i - 1] + "_" + values[i]), out, new ItemUseInfo(in)).registerManual();
        }
        ItemInstance in = new ItemInstance(Tiles.plain, 1, values.length - 1);
        new BasicRecipe(createRes("plain_" + values[values.length - 1] + "_" + values[0]), plain, new ItemUseInfo(in)).registerManual();
    }
}
